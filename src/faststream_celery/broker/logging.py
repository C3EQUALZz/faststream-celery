import logging
from functools import partial
from typing import TYPE_CHECKING, Any

from faststream_celery._internal import (
    DefaultLoggerStorage,
    get_broker_logger,
    make_logger_state,
)

if TYPE_CHECKING:
    from faststream_celery._internal import ContextRepo, LoggerProto


class CeleryParamsStorage(DefaultLoggerStorage):
    def __init__(self) -> None:
        super().__init__()

        self._max_queue_name = 4

        self.logger_log_level = logging.INFO

    def set_level(self, level: int) -> None:
        self.logger_log_level = level

    def register_subscriber(self, params: dict[str, Any]) -> None:
        self._max_queue_name = max(
            (
                self._max_queue_name,
                len(params.get("queue", "")),
            ),
        )

    def get_logger(self, *, context: "ContextRepo") -> "LoggerProto":
        message_id_ln = 10

        if not (lg := self._get_logger_ref()):
            lg = get_broker_logger(
                name="celery",
                default_context={
                    "queue": "",
                },
                message_id_ln=message_id_ln,
                fmt=(
                    "%(asctime)s %(levelname)-8s - "
                    f"%(queue)-{self._max_queue_name}s | "
                    f"%(message_id)-{message_id_ln}s "
                    "- %(message)s"
                ),
                context=context,
                log_level=self.logger_log_level,
            )
            self._logger_ref.add(lg)

        return lg


make_celery_logger_state = partial(
    make_logger_state,
    default_storage_cls=CeleryParamsStorage,
)
